...
import { FireBall } from '../Attack/FireBall';
...
export class PlayerAttack extends Component {
    ...
    private spawnFireBall(){
        if(!this.fireBallPrefab){
            console.warn("未设置火球预制体")
            return
        }

        // 寻找最近的敌人
        const nearest = this.findNearestEnemy()
        if(!nearest) return // 没有敌人则不生成火球

        const double = this.playerController.hasDoubleFireball()
        ...
        for(let i = 0; i < count; i++){
            ...
            if(fireballScript) fireballScript.init(nearest, attackValue)
        }
    }
    ...
    private findNearestEnemy():Node | null{
        ...
        // 查找本地敌人
        const enemySpawner = canvas.getChildByName('EnemySpawner')
        for(const child of enemySpawner.children){
            if(child.name === 'Enemy' && child.isValid){
        ...
        // 查找网络敌人（联机模式）
        const networkManager = canvas.getChildByName('NetworkManager')
        if(networkManager){
            // 遍历Canvas下所有NetworkEnemy开头的节点
